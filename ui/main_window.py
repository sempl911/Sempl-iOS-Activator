...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.progress_max = 8  # Увеличиваем до 8 (добавили Developer Mode)
    ...
    def setup_ui(self):
        ...
        dev_mode_layout.setSpacing(10)
        
        self.dev_mode_checkbox = QCheckBox("🔓 Включать Developer Mode автоматически")
        self.dev_mode_checkbox.setChecked(True)
        self.dev_mode_checkbox.setStyleSheet("color: #cccccc; font-size: 13px;")
        self.dev_mode_checkbox.toggled.connect(self.toggle_dev_mode)
        
        dev_mode_layout.addWidget(self.dev_mode_checkbox)
        dev_mode_layout.addStretch()
        
        activation_layout.addLayout(dev_mode_layout)
    ...
    def toggle_dev_mode(self, checked):
        """Включение/выключение автоматического Developer Mode"""
        if self.activator:
            self.activator.set_dev_mode(checked)
            status = "включено" if checked else "выключено"
            self.append_log(f"🔓 Developer Mode автоматическое включение: {status}")
    ...
    def start_activation(self):
        ...
        # Сохраняем состояние переключателя в активатор
        self.activator.set_dev_mode(self.dev_mode_checkbox.isChecked())
    ...
    def on_activation_progress(self, msg):
        ...
        if "1/" in msg:
            ...
            self.signals.progress_update.emit(7, "Developer Mode")
        elif "7/" in msg:
            self.signals.progress_update.emit(8, "Перезагрузка")
