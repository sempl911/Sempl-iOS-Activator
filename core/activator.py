...
class Activator:
    ...
    def __init__(self):
        ...
        self.enable_dev_mode = True
        
    def set_progress_callback(self, callback: Callable[[str], None]):
        self.progress_callback = callback
    
    def set_dev_mode(self, enabled: bool):
        """Включает/выключает автоматическое включение Developer Mode"""
        self.enable_dev_mode = enabled
    ...
    def _run_command(self, cmd: list, timeout: int = 60) -> Optional[subprocess.CompletedProcess]:
        try:
    ...
    def _run_command_background(self, cmd: list):
        ...
        try:
            ...
            self._log(f"   ⚠️ Ошибка фонового запуска: {e}")
            return False
    ...
    def _install_profile(self, profile_path: str) -> bool:
        ...
        self._log("   📲 Приложение установится автоматически через профиль")
        
        result = self._run_command(["pymobiledevice3", "profile", "install", profile_path], timeout=60)
        if result and result.returncode == 0:
            self._log("   ✅ Профиль установлен")
            self._log("   ⏳ Приложение устанавливается через MDM...")
            time.sleep(5)  # Даем время на установку через MDM
            return True
        else:
            self._log("   ❌ Ошибка установки профиля")
            return False
    
    def _install_app_managed(self) -> bool:
        """
        Установка приложения через MDM как управляемое.
        Проверяет, установилось ли через профиль, если нет - устанавливает принудительно.
        """
        self._log("6/8 Установка управляемого приложения...")
        
        if not self.app_path or not os.path.exists(self.app_path):
            self._log("   ❌ Приложение не найдено")
            return False
        
        # Сначала проверяем, установилось ли уже через профиль
        self._log("   🔍 Проверка установки через MDM...")
        result = self._run_command(["pymobiledevice3", "apps", "list"], timeout=30)
        
        if result and result.returncode == 0 and self.app_bundle_id in result.stdout:
            self._log("   ✅ Приложение уже установлено через MDM профиль!")
            return True
        
        # Если не установилось - пробуем установить через apps install с флагом managed
        self._log("   📲 Принудительная установка как управляемое...")
        cmd = ["pymobiledevice3", "apps", "install", self.app_path, "--managed"]
        result = self._run_command(cmd, timeout=180)
        
        if result and result.returncode == 0:
            self._log("   ✅ Приложение установлено как управляемое!")
            return True
        
        # Если managed не работает - пробуем с developer флагом
        self._log("   🔄 Пробуем с developer флагом...")
        cmd = ["pymobiledevice3", "apps", "install", self.app_path, "--developer"]
        result = self._run_command(cmd, timeout=180)
        
        if result and result.returncode == 0:
            self._log("   ✅ Приложение установлено с developer флагом!")
            return True
        
        # Последняя попытка - обычная установка
        self._log("   🔄 Пробуем обычную установку...")
    ...
    def _enable_developer_mode(self) -> bool:
        """Автоматически включает Developer Mode на устройстве"""
        self._log("   🔓 Включение Developer Mode...")
        
        try:
            # Проверяем статус Developer Mode
            cmd_status = ["pymobiledevice3", "amfi", "developer-mode-status"]
            result = self._run_command(cmd_status, timeout=10)
            
            if result and result.returncode == 0 and result.stdout:
                if "enabled" in result.stdout.lower() or "on" in result.stdout.lower():
                    self._log("   ✅ Developer Mode уже включен")
                    return True
            
            # Отправляем сигнал включения в фоне (не ждем завершения)
            self._log("   📤 Отправка сигнала включения...")
            subprocess.Popen(
            ...
            self._log("   ✅ Сигнал отправлен")
            self._log("   📱 На телефоне появится запрос на включение режима")
            
            return True
            
        except Exception as e:
            self._log(f"   ⚠️ Ошибка отправки сигнала: {e}")
            return True
    ...
    def _wait_for_device(self, timeout: int = 180) -> bool:
    ...
    def activate_and_bypass(
        ...
        # Шаг 5: Установка профиля (приложение устанавливается автоматически)
        if not self._install_profile(profile_path):
            return False
        time.sleep(2)
        
        # Шаг 6: Проверка установки приложения (если не установилось через профиль)
        if install_app and self.app_path and os.path.exists(self.app_path):
            self._install_app_managed()
        else:
            self._log("6/8 Установка приложения пропущена")
        time.sleep(1)
        
        # Шаг 6.5: Включение Developer Mode (если включено в настройках)
        if self.enable_dev_mode:
            self._log("6.5/8 Включение Developer Mode...")
            self._enable_developer_mode()
            time.sleep(2)
        else:
            self._log("6.5/8 Включение Developer Mode пропущено")
        ...
        if self.enable_dev_mode:
            self._log("🔓 Developer Mode: сигнал отправлен")
        self._log("")
